...
> 显示文件或目录所占用的磁盘空间
``` sh
# 显示当前目录的大小，但是不会列出目录中的每一个文件
du -sh
# 列出下载位置下每个目录和每个文件所占的容量
du -sh ~/Downloads
# 列出下载位置下每个目录和每个文件所占的容量
du -sh ~/Downloads/*
# 列出当前文件夹下 a.txt 和 b.txt 的大小
du -sh a.txt b.txt
```
::: tip du常用参数
- `-h` --human-readable 使用人类可读的格式(预设值是不加这个选项的...)
...
### ncdu <Badge text="强烈推荐"/>
> [NCurses Disk Usage](http://dev.yorhel.nl/ncdu)，简称`ncdu`，该命令需要安装。扫描你当前文件目录的大小,并且自动会把目录从大到小排序，并且可以基于图形界面的操作
``` sh
# ubuntu
...
```
::: tip ncdu键盘快捷键
感谢同学的[分享](https://mp.weixin.qq.com/s/TaFAiHW7R3neZbt8Dn9j6w)
...
- [如何在 Linux 中查看目录大小？](https://mp.weixin.qq.com/s/TaFAiHW7R3neZbt8Dn9j6w)
